var game;
(function (game) {
    class Main_webgl_animation_skinning_morph {
        constructor() {
            ...
            this.domMemInfo = document.getElementById('meminfo');
        ...
        loadSuccess(geometry, materials) {
        ...
        initGUI() {
            ...
            var clipControl = function clipControl(gui, mixer, clip, rootObjects) {
                var folder = gui.addFolder("Clip '" + clip.name + "'"), rootNames = objectNames(rootObjects), rootName = rootNames[0], root = rootObjects[0], action = null, API = {
                    'play()': function play() {
            ...
            var memoryControl = function (gui, mixer, clips, rootObjects) {
                var clipNames = objectNames(clips), rootNames = objectNames(rootObjects);
                var folder = gui.addFolder("Memory Management"), clipName = clipNames[0], clip = clips[0], rootName = rootNames[0], root = rootObjects[0], API = {
            ...
            memoryControl(gui, this.mixer, [this.bonesClip, this.facesClip], [this.mesh, this.mesh2]);
        }
    }
    game.Main_webgl_animation_skinning_morph = Main_webgl_animation_skinning_morph;
})(game || (game = {}));
